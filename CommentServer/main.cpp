...
public:
    ...
    HttpResponse& AddHeader(string name, string value) {
        ...
        _headers.emplace_back(move(name), move(value));
        return *this;
    }
    HttpResponse& SetContent(string a_content) {
        _response = move(a_content);
        return *this;
    }
...
ostream& operator << (ostream& output, const HttpResponse& resp) {
    output << "HTTP/1.1 " << resp._code << "\n";
    for(const auto& header : resp._headers) {
        output << header.first << ": " << header.second << "\n";
    }
    if (!resp._response.empty()) {
        output << "Content-Length" << ": " << resp._response.size() << "\n";
    }
    output << "\n" << resp._response;
    return output;
}
    ...
    HttpResponse ServeRequest(const HttpRequest& req) {
        if (req.method == "POST") {
            ...
            if (req.path == "/user_comments") {
                ...
                auto size = to_string(response.size());
                return HttpResponse (HttpCode::Ok).SetContent(response);
...
void Test(CommentServer& srv, const HttpRequest& request, const ParsedResponse& expected) {
    stringstream ss;
    auto httpResp = srv.ServeRequest(request);
    ss << httpResp;
    ParsedResponse resp;
    ss >> resp;
