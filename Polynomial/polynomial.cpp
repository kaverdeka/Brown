...
class Polynomial {
    ...
    class IndexProxy {
    public:
        IndexProxy(Polynomial<T>& poly, size_t degree) :
            _poly(poly), _degree(degree) {}

        T& operator=(const T& t) {
            if((_degree < _poly.coeffs_.size()) || t == T()) {
                _poly.coeffs_[_degree] = t;
                if(t == T())
                    _poly.Shrink();

                return _poly.coeffs_[_degree];
            }

            _poly.coeffs_.resize(_degree + 1);
            _poly.coeffs_[_degree] = t;
            return _poly.coeffs_[_degree];
        }
...
void TestNonconstAccess() {
    ...
    poly2[0] = 1;
    ASSERT_EQUAL(poly2[0], 1);
    poly2[0] = 5;
    ASSERT_EQUAL(poly2[0], 5);
    poly2[0] = 0;
    ASSERT_EQUAL(poly2[0], 0);
}
