...
public:
    ...
    struct WriteAccess {
        lock_guard<mutex> writeLock;
        V& ref_to_value;
    };

    struct ReadAccess {
        lock_guard<mutex> readLock;
        const V& ref_to_value;
    };

    explicit ConcurrentMap(size_t bucket_count) :
            _maps(bucket_count), _mutexes(bucket_count), _bucketCount(bucket_count) {
    }

    WriteAccess operator[](const K& key) {
        auto idx = getIdx(key);
        return {lock_guard(_mutexes[idx]), _maps[idx][key] };
    }

    ReadAccess At(const K& key) const {
        auto idx = getIdx(key);
        return {lock_guard(_mutexes[idx]), _maps[idx].at(key) };
    }

    bool Has(const K& key) const {
        auto idx = getIdx(key);
        lock_guard lock(_mutexes[idx]);
        return _maps[idx].count(key);
    }

    MapType BuildOrdinaryMap() const {
        MapType result;
        for(size_t idx = 0; idx < _maps.size(); ++idx) {
            lock_guard lock(_mutexes[idx]);
            result.insert(_maps.at(idx).begin(), _maps.at(idx).end());
        }
        return  result;
    }

private:
    const size_t getIdx(const K& key) const {
        return _hasher(key) % _bucketCount;
    }

    Hash _hasher;
    vector<MapType> _maps;
    mutable vector<mutex> _mutexes;
    //mutable vector<mutex> _readMutexes;
